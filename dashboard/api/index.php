<?php
	...
	function verifyCommand($recv) {
		...
		if (!isset($voice_found)) echo printMsg('error', 'Não entendi. Pode falar novamente?');
	}
	...
	function printMsg($status, $message) {
		...
		if (!empty($_GET['isAjax']) && $_GET['isAjax'] == 1 && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) &&
			strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && !empty($_GET)) {
