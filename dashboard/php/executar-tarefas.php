<?php
	...
	function acionarDispositivo($disp, $action) {
		...
		if ($action == 2) $send = $dev['device_cmd_on']; // Ligar
		else if ($action == 3) $send = $dev['device_cmd_off']; // Desligar
		else if ($action == 4) $send = $dev['device_cmd_toggle']; // Abrir
	...
	while ($linha = mysql_fetch_array($resultado, MYSQL_ASSOC)) {
		...
		if ($hora_atual == $linha['task_time'] && isset($linha['task_device_id'])) {
			...
			if ($linha['task_frequency'] == 1) {
				acionarDispositivo($linha['task_device_id'], $linha['task_action']);
			} else if ($linha['task_frequency'] == 2 && $dia_util == 1) {
				acionarDispositivo($linha['task_device_id'], $linha['task_action']);
			} else if ($linha['task_frequency'] == 3 && $dia_util == 0) {
				acionarDispositivo($linha['task_device_id'], $linha['task_action']);
			} else if ($linha['task_frequency'] == 4 && $data_atual == $linha['task_date']) {
				acionarDispositivo($linha['task_device_id'], $linha['task_action']);
			}
		}
	}
